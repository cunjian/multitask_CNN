...
import sys
sys.path.append("..")
from common.utils import BBox, drawLandmark,drawLandmark_multiple, check_bbox, processImage,scale,drawAttribute
...
class Predict(object):
	...
	def forward_gender(self, face, layer='prob_gender'):
		'''
	...
	def forward_age(self, face, layer='prob_age'):
		'''
		face has been preprocessed to (1,C,H,W) before feeding to cnn.
		'''
		out = self.cnn.forward_all(data=np.asarray([face]))
...
if __name__ == '__main__':
	...
	net = Predict('../train_multitask/deploy.prototxt', sys.argv[1])
