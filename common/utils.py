...
def drawAttribute(img, bbox, gender,age):
	'''
	Input:
	...
	#	cv2.circle(img, (int(x), int(y)), 3, (0,255,0), -1)
        if gender.argmax()==0:
                # -1->female, 1->male; -1->old, 1->young
                cv2.putText(img, 'F:'+str(round(gender[0]*100))+'%', (int(bbox.left), int(bbox.top)),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)

        else:
                cv2.putText(img, 'M:'+str(round(gender[1]*100))+'%', (int(bbox.left), int(bbox.top)),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0),2)
        if age.argmax()==0:
                cv2.putText(img, 'O:'+str(round(age[0]*100))+'%', (int(bbox.right), int(bbox.bottom)),cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 0), 2)
        else:
                cv2.putText(img, 'Y:'+str(round(age[1]*100))+'%', (int(bbox.right)-40, int(bbox.bottom)),cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 0), 2)
	return img
